...
from keras.layers import Conv1D, MaxPooling1D, LSTM, Dense, Flatten, LeakyReLU, Dropout
...
# 1D-CNN 层
model.add(Conv1D(filters=56, kernel_size=2, activation='relu',
                 kernel_regularizer=regularizers.l1_l2(l1=1.0805843791263926e-06, l2=4.540555326102059e-06),
                 input_shape=(9, 1)))
...
# 额外的 1D-CNN 层
model.add(Conv1D(filters=16, kernel_size=4, padding='same', strides=1))
model.add(LeakyReLU(alpha=0.3))
model.add(MaxPooling1D(pool_size=2))

# LSTM 层
model.add(LSTM(units=70, return_sequences=True, activation='relu',
               kernel_regularizer=regularizers.l1_l2(l1=4.508324743209948e-06, l2=5.9424260748720894e-06),
               dropout=0.28656904517561677, recurrent_dropout=0.20555067255968323))

model.add(LSTM(units=65, return_sequences=False, activation='relu',
               dropout=0.1403052882218182, recurrent_dropout=0.13291036538230594))

# Dropout 层
model.add(Dropout(0.17655838795982484))
...
# 编译模型
optimizer = Adam(learning_rate=0.0005470817929946867)
...
# 保存模型
model.save('my_trained_model_accuracy.h5')
